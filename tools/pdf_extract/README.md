...
│   └── confusables.py      # flag_confusables (vi/vĩ, sỉ/sĩ...)
├── requirements.txt
└── README.md
```
...
- **PDF scan (ảnh, không có text layer)** → text/table sẽ rỗng. Cần OCR trước (vd dùng Tesseract). Tool chỉ extract image chứ không OCR.
