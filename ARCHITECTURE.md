...
## 2. Luồng xử lý (do Claude trong IDE thực hiện)

```
┌─────────────────────────────────────────────────────────────┐
│  STAGE 0: audio-to-transcript  (TÙY CHỌN — cache-aware)     │
│                                                              │
│  ⚠️ CHỈ chạy khi metadata.yaml có media_types: ["audio"]     │
│  Môn không có cờ này → BỎ QUA Stage 0, vào thẳng Stage 1.   │
│                                                              │
│  Cho mỗi audio trong lectures/audio/ và exercises/audio/:    │
│    1. Tính SHA-256 của file audio                            │
│    2. Kiểm tra transcript MD tương ứng đã có chưa            │
│    3. Đọc front-matter MD → so source_hash                   │
│    4. Khớp → SKIP                                            │
│       Khác/không có → transcribe (Whisper qua terminal)      │
│    5. Ghi _transcript.md với front-matter mới                │
│    6. Nếu transcript > 2000 token → tạo _transcript_summary │
└──────────────────────────┬───────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────┐
...
│  • Nếu môn có audio: đọc _transcript.md liên quan            │
│  • Với mỗi câu, viết theo template 8 phần (+🎧 nếu audio)    │
...
│   └── audio/                          ← (tùy chọn) MP3 bài giảng — chỉ khi metadata có media_types: [audio]
├── exercises/
│   ├── pdf/
│   │   └── de-on-tap-chuong-1.pdf
│   ├── md/
│   │   ├── de-on-tap-chuong-1.md
│   │   └── unit-1-listening_transcript.md  ← (tùy chọn) transcript từ audio
│   └── audio/                          ← (tùy chọn) MP3 đề listening
│       └── unit-1-listening.mp3
├── solutions/
...
### Cache cho Audio (tương tự PDF)

Mỗi file transcript có front-matter:

```yaml
---
source_audio: "unit-1-listening.mp3"
source_hash: "sha256:9a8b7c..."
transcribed_at: "2026-05-10T14:00:00Z"
---
```

Logic cache giống hệt PDF:
```bash
sha256sum subjects/<môn>/exercises/audio/<file>.mp3
```
- Khớp `source_hash` → skip.
- Khác / transcript chưa có → transcribe mới.
- `--force` → luôn transcribe lại.
...
- **Video (multi-modal)**: `media_types` có thể mở rộng thành `["audio", "video"]` — thêm skill `video-to-transcript`, folder `video/` song song với `audio/`.
- **Multi-language audio**: Dùng `audio_language` trong metadata để hỗ trợ audio nhiều ngôn ngữ (EN, JA, ZH...).
